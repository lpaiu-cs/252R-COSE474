...
# cross entropy loss
########################################
def softmax(W, b, x_i, y_i):
    s = W @ x_i.T + b
    s = s - np.max(s)
    exp_s = np.exp(s)
    return exp_s[y_i] / np.sum(exp_s)

def cross_entropy_softmax_loss(Wb, x, y, num_class, n, feat_dim):
    '''return cross entropy loss

        n: dataset_size\\
        num_class: the number of classify\\
        feat_dim: the dimension of input data.\\
        x: input data (n, feat_dim)\\
        y: output data (n,)\\
        Wb: W + b (num_class, feat_dim) + (num_class,)
    '''
    # 처음에 파싱 방법을 잘못 택해서 정확도가 50 언저리만 나왔었다.
    ...
    loss = 0
    for i in range(n):
        loss -= np.log(softmax(W, b, x[i,:], y[i]))
